...
def test_tools_use_one_library_without_changing_import_search(monkeypatch):
    ...
    assert generate_config._detect_cwc_filter_column is pipeline_config._detect_cwc_filter_column
...
def test_cli_writes_config_from_extracted_data(monkeypatch, tmp_path, report_id):
    ...
    args = [
        "generate_config", "--workspace-id", "workspace", "--dataset-id", "dataset",
        "--tenant-id", "tenant", "--client-id", "client", "--client-secret", "secret",
        "--admin-client-id", "admin", "--admin-client-secret", "admin-secret",
        "--catalog", "catalog", "--schema", "schema", "--output", str(output),
    ]
    if report_id:
    ...
    mocks["extract_relationships"].assert_called_once_with("token", "workspace", "dataset")
    mocks["trigger_admin_scan"].assert_called_once_with("admin-token", "workspace")
    mocks["build_config"].assert_called_once_with(
        relationships, measures, tables, report if report_id else None,
        catalog="catalog", schema="schema",
    )
    if report_id:
        mocks["extract_report_definition"].assert_called_once_with(
            "token", "workspace", report_id, tenant_id="tenant",
