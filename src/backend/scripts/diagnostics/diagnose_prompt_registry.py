...
* **Inside the deployed Databricks App** (authenticates as the app SP — the
  identity that actually fails): copy this script there explicitly, then run
      python backend/scripts/diagnostics/diagnose_prompt_registry.py <catalog>.<schema>
* **Locally** with a PAT/SP creds in the environment
  (DATABRICKS_HOST + DATABRICKS_TOKEN, or DATABRICKS_CLIENT_ID/SECRET):
      python src/backend/scripts/diagnostics/diagnose_prompt_registry.py <catalog>.<schema>
