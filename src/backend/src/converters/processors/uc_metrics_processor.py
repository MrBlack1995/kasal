"""
UC Metrics Store Processor
Converts KBI definitions to Unity Catalog metrics store format
"""

import logging
from typing import Dict, List, Any, Optional
from converters.models.kbi import KBI

logger = logging.getLogger(__name__)

class UCMetricsProcessor:
    """Processor for generating Unity Catalog metrics store definitions"""

    def __init__(self, dialect: str = "spark"):
        self.dialect = dialect

    def process_kbi_to_uc_metrics(self, kbi: KBI, yaml_metadata: Dict[str, Any]) -> Dict[str, Any]:
        """Convert a single KBI to UC metrics format"""

        # Extract basic information
        measure_name = kbi.technical_name or "unnamed_measure"
        description = kbi.description or f"UC metrics definition for {measure_name}"

        # Build source table reference
        source_table = self._build_source_reference(kbi.source_table, yaml_metadata)
    ...
    def _build_filter_conditions(self, kbi: KBI, yaml_metadata: Dict[str, Any]) -> Optional[str]:
        ...
        for filter_condition in kbi.filters:
    ...
    def _build_measure_expression(self, kbi: KBI) -> str:
        """Build the measure expression based on aggregation type and formula"""

        aggregation_type = kbi.aggregation_type.upper() if kbi.aggregation_type else "SUM"
        formula = kbi.formula or "1"
    ...
    def _build_measure_expression_with_filter(self, kbi: KBI, specific_filters: Optional[str]) -> str:
    ...
    def _build_exception_aggregation_with_window(self, kbi: KBI, specific_filters: Optional[str]) -> tuple[str, dict]:
    ...
    def process_consolidated_uc_metrics(self, kbi_list: List[KBI], yaml_metadata: Dict[str, Any]) -> Dict[str, Any]:
        """Convert multiple KBIs to a consolidated UC metrics format"""

        # Separate KBIs by type
        constant_selection_kbis = [kbi for kbi in kbi_list if hasattr(kbi, 'fields_for_constant_selection') and kbi.fields_for_constant_selection]
        regular_kbis = [kbi for kbi in kbi_list if not (hasattr(kbi, 'fields_for_constant_selection') and kbi.fields_for_constant_selection)]
        ...
        for kbi in kbi_list:
            ...
            has_constant_selection = hasattr(kbi, 'fields_for_constant_selection') and kbi.fields_for_constant_selection
    ...
    def _extract_common_filters(self, kbi_list: List[KBI], yaml_metadata: Dict[str, Any]) -> Optional[str]:
    ...
    def _get_kbi_specific_filters(self, kbi: KBI, common_filters: Optional[str], yaml_metadata: Dict[str, Any]) -> Optional[str]:
    ...
    def _build_constant_selection_uc_metrics(self, kbi: KBI, yaml_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _build_consolidated_constant_selection_uc_metrics(self, constant_selection_kbis: List[KBI], yaml_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        source_tables = [kbi.source_table for kbi in constant_selection_kbis if kbi.source_table]
        ...
        for kbi in constant_selection_kbis:
            # Build KBI-specific filters for FILTER clause
            kbi_specific_filters = []
            for filter_condition in kbi.filters:
                if filter_condition == "$query_filter":
