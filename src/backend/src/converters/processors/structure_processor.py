"""
Structure Processor for SAP BW Time Intelligence and Reusable Calculations
...
import re
from converters.models.kbi import KBIDefinition, KBI, Structure


class StructureProcessor:
    """Processes KBIs with applied structures to create combined measures"""
    
    def __init__(self):
        self.processed_definitions: List[KBIDefinition] = []
    
    def process_definition(self, definition: KBIDefinition) -> KBIDefinition:
        """
        Process a KBI definition and expand KBIs with applied structures
        ...
        for kbi in definition.kbis:
            if kbi.apply_structures:
                ...
                combined_kbis = self._create_combined_measures(
                    kbi, definition.structures, kbi.apply_structures, definition
                )
                expanded_kbis.extend(combined_kbis)
            else:
                # No structures applied, keep original KBI
                expanded_kbis.append(kbi)
        
        # Create new definition with expanded KBIs
        expanded_definition = KBIDefinition(
        ...
        for struct_name in structure_names:
            ...
            if structure.formula:
                ...
                if structure.filters:
                    ...
                    # Create a temporary KBI with just structure filters to resolve them
                    temp_kbi = KBI(
                        description="temp",
                        technical_name="temp", 
                        formula="temp",
                    ...
                    resolved_structure_filters = filter_resolver.resolve_filters(definition, temp_kbi)
            ...
            combined_measures.append(combined_kbi)
    ...
    def validate_structures(self, definition: KBIDefinition) -> List[str]:
        """
        ...
        # Check KBI structure references
        for kbi in definition.kbis:
            if kbi.apply_structures:
                for struct_name in kbi.apply_structures:
                    if struct_name not in definition.structures:
                        errors.append(f"KBI '{kbi.technical_name or kbi.description}' references undefined structure: {struct_name}")
