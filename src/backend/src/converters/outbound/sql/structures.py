...
import re
import logging
from converters.base.models import KPI, KPIDefinition, Structure
...
class SQLStructureExpander:
        ...
        for kpi in definition.kpis:
            if kpi.apply_structures:
                ...
                combined_measures = self._create_combined_sql_measures(
                    kbi, sql_structures, kpi.apply_structures, definition, options
                )
    ...
    def _convert_filters_to_sql(self, filters: List[str], definition: KPIDefinition) -> List[str]:
        ...
        from converters.outbound.sql.aggregations import SQLFilterProcessor
    ...
    def kpi: KPI, definition: KPIDefinition, options: SQLTranslationOptions) -> SQLMeasure:
        """Convert a single KBI to SQL measure"""
        from converters.outbound.sql.aggregations import detect_and_build_sql_aggregation
        ...
        sql_measure = SQLMeasure(
            name=kbi.description or kpi.technical_name or "Unnamed Measure",
            description=kbi.description or "",
            sql_expression=sql_expression,
            aggregation_type=agg_type,
            source_table=kbi.source_table or "fact_table",
            source_column=kbi.formula if self._is_simple_column_reference(kpi.formula) else None,
            filters=sql_filters,
    ...
    def kpi: KPI, sql_structure: SQLStructure, combined_name: str, definition: KPIDefinition) -> KBI:
