...
from converters.models.kbi import KBIDefinition, KBI, QueryFilter, Structure


class YAMLKBIParser:
    def __init__(self):
        self.parsed_definitions: List[KBIDefinition] = []
    
    def parse_file(self, file_path: Union[str, Path]) -> KBIDefinition:
        """Parse a single YAML file containing KBI definitions."""
        path = Path(file_path)
        if not path.exists():
    ...
    def parse_directory(self, directory_path: Union[str, Path]) -> List[KBIDefinition]:
    ...
    def _parse_yaml_data(self, data: Dict[str, Any]) -> KBIDefinition:
        """Convert raw YAML data to KBIDefinition model."""
        # Parse query filters
        query_filters = []
    ...
    def get_all_kbis(self) -> List[tuple[KBIDefinition, KBI]]:
