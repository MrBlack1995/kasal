...
class SQLQuery(BaseModel):
    ...
    def _generate_formatted_sql(self) -> str:
        ...
        # LIMIT clause (dialect-specific)
        if self.limit_clause:
            if self.dialect == SQLDialect.SQLSERVER:
                # SQL Server uses TOP at the beginning
                if lines and lines[0].startswith("SELECT"):
                    lines[0] = lines[0].replace("SELECT", f"SELECT TOP {self.limit_clause}")
            else:
    ...
    def _generate_compact_sql(self) -> str:
        ...
        if self.limit_clause:
            if self.dialect == SQLDialect.SQLSERVER:
                sql_parts.append(f"TOP {self.limit_clause}")
            else:
...
class SQLDefinition(BaseModel):
    """SQL equivalent of KBIDefinition"""
    ...
    original_kbis: List[KBI] = Field(default=[])
    ...
    def get_full_table_name(self, table_name: str) -> str:
        ...
        if self.dialect == SQLDialect.BIGQUERY:
            return ".".join(parts)
        elif self.dialect in [SQLDialect.SQLSERVER, SQLDialect.DATABRICKS]:
            return ".".join(parts)
        else:
