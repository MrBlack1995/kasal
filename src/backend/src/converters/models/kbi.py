"""Core data models for KBI (Key Business Indicator) measure conversion"""

from typing import List, Dict, Any, Optional, Union
from pydantic import BaseModel, Field, ConfigDict


class KBIFilter(BaseModel):
    """Filter definition for KBI measures"""
    field: str
    operator: str
    value: Any
...
class Structure(BaseModel):
    ...
    applied to multiple KBIs (e.g., YTD, QTD, prior year comparisons).
    """
    description: str
...
class KBI(BaseModel):
    """
    Key Business Indicator (KBI) model.
    ...
    # Structure application - list of structure names to apply to this KBI
...
class KBIDefinition(BaseModel):
    """
    Complete KBI definition from YAML input.

    Contains the full specification including metadata, filters,
    structures, and all KBI measures.
    """
    description: str
    technical_name: str
...
class DAXMeasure(BaseModel):
    ...
    description: str
    dax_formula: str
    original_kbi: Optional[KBI] = None
