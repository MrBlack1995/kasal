...
- A request that REPEATS one already answered in this conversation, or only asks for that answer in another form — "as a table", "shorter", "in German", "just the numbers" — is not new work either: return null, and the answer is reshaped from the conversation. Only an explicit ask to redo the work ("run it again", "refresh", "get the latest") routes. (When the previous answer came from a capability that holds a conversation, the rule in the next section applies instead.)
