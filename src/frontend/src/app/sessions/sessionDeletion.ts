...
export async function deleteArchivedSession(session: WorkspaceSession, groupId: string) {
  ...
  const tab = session.mode === 'chat' ? undefined : useTabManagerStore.getState().getTab(session.id);
  ...
  if (tab) {
    const wasActive = useTabManagerStore.getState().activeTabId === tab.id;
    useTabManagerStore.getState().closeTab(tab.id);
