import type { TabData } from '../../store/tabManager';
...
export function isEmptySessionDraft(tab: TabData) {
...
export function collectSessions(
  chats: ChatSession[], tabs: TabData[], groupId: string,
  permissions: { crew: boolean; flow: boolean },
): WorkspaceSession[] {
  const builderChatIds = new Set(tabs.map(tab => tab.chatSessionId));
  return [
    ...chats.filter(chat => (!chat.groupId || chat.groupId === groupId) && !builderChatIds.has(chat.id)).map(chat => ({
    ...
    ...tabs.filter(tab => tab.group_id === groupId && permissions[tab.viewMode] && !isEmptySessionDraft(tab)).map(tab => ({
