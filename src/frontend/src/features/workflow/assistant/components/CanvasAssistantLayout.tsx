...
export function CanvasAssistantLayout({ composer, response, responseKey, hasMessages, busy, dark, onNewChat, showEarlier, onToggleEarlier, onHide }: Props) {
  ...
  const focused = useUILayoutStore(state => state.assistantResponseFocused) && visible;
  ...
  useLayoutEffect(() => { setHost(document.getElementById('builder-assistant-response-host')); }, [visible, side, focused]);
  useEffect(() => {
  ...
  const panel = <Box role="region" aria-label="Kasal responses" sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, ...kasalStageSurface(dark), color: 'text.primary', ...(fullscreen || focused ? { flex: 1, height: 'auto', maxWidth: 1000, width: '100%', mx: 'auto', py: { xs: 2, sm: 3 }, boxSizing: 'border-box' } : {}) }}>
  ...
  const toolbar = (
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 0.25, mb: 0.5, pointerEvents: 'auto', width: 'fit-content', ml: 'auto', bgcolor: dark ? '#232930' : '#FFFFFF', borderRadius: 2, px: 0.5, boxShadow: '0 2px 8px rgba(16,24,40,.06)' }}>
        <Button ref={restoreRef} color="inherit" size="small" startIcon={<MessageSquare size={14} />} onClick={() => visible ? closeResponse() : showResponse()} sx={{ fontSize: 11, px: 1 }}>{visible ? 'Hide response' : 'Responses'}</Button>
        <Tooltip title="New Chat"><span><IconButton aria-label="New Chat" size="small" disabled={busy} onClick={() => { closeResponse(); onNewChat(); }}><SquarePen size={15} /></IconButton></span></Tooltip>
        {onHide && <Tooltip title="Hide assistant"><IconButton aria-label="Hide assistant" size="small" onClick={() => { closeResponse(); onHide(); }}><X size={15} /></IconButton></Tooltip>}
      </Box>
  );
  const focusReader = <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
    {panel}
    <Box sx={{ width: '100%', maxWidth: 760, mx: 'auto', px: 1.5, pb: 1.5, flexShrink: 0, boxSizing: 'border-box' }}>
      {toolbar}
  ...
  return <>
    ...
    {visible && !fullscreen && host && createPortal(focused ? focusReader : panel, host)}
    <Dialog fullScreen open={fullscreen && visible} onClose={returnToSidebar} PaperProps={{ 'aria-label': 'Full screen responses', sx: { ...kasalStageSurface(dark), border: 0, boxShadow: 'none' } }}>
      {fullscreen && <>
        {panel}
