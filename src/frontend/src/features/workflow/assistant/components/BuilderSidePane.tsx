...
export default function BuilderSidePane({ tabs, active, onSelect, onClose, dark, canvasHost }: {
  ...
  return <Box className="kasal-chat-root" data-theme={dark ? 'dark' : 'light'}
    ...
    <Box data-builder-pane-tabs sx={{ display: 'flex', alignItems: 'center', height: 44, flexShrink: 0, px: 1, pointerEvents: 'auto', ...kasalStageSurface(dark) }}>
      <Tabs value={active} onChange={(_, id: BuilderPaneId) => onSelect(id)} variant="scrollable" scrollButtons="auto" allowScrollButtonsMobile
        ...
        {(['canvas', ...tabs.map(tab => tab.id)] as BuilderPaneId[]).map(id => <Tab key={id} value={id} label={labelFor(id)}
          sx={id === 'canvas' ? { position: 'sticky', left: 0, zIndex: 1, ...kasalStageSurface(dark), '&.Mui-selected': { backgroundImage: 'none' } } : undefined}
          id={`builder-view-tab-${id}`} aria-controls={id === 'canvas' ? undefined : `builder-view-panel-${id}`} />)}
      </Tabs>
      {active !== 'canvas' && <IconButton size="small" aria-label={`Close ${labelFor(active)}`} onClick={() => onClose(active)}><X size={16} /></IconButton>}
    </Box>
    {/* Keep opened forms mounted when another tab is selected, retaining drafts. */}
    {tabs.map(tab => <Box key={tab.id} role="tabpanel" id={`builder-view-panel-${tab.id}`} aria-labelledby={`builder-view-tab-${tab.id}`} hidden={active !== tab.id}
      sx={{ display: active === tab.id ? 'flex' : 'none', flex: 1, minHeight: 0, minWidth: 0, overflow: 'hidden', pointerEvents: 'auto', ...kasalStageSurface(dark) }}>
      {'editor' in tab ? <Box sx={{ width: '100%', height: '100%', minHeight: 0, overflow: 'hidden' }}
